...
- Connectors: decode `startItem`/`endItem` `position.x` / `y` as numbers, numeric strings, or percentage strings such as `50%` (relative anchor; normalized to 0..1 with optional `x_raw`/`y_raw` on the wire)
